...
Thư mục này chứa cert mTLS của **server** (`service_id = "vault-server"`). Mọi file
...
Script sinh sẵn vào đây (và cả bên `client/`):
...
Sau đó `python -m app.main` khởi động ở trạng thái **ACTIVE** (có cert, không có
bootstrap) và phục vụ mTLS mà không cần Trust Service.
...
- `bootstrap.txt` - file bootstrap cert lần đầu, do Trust Service cấp cho đúng
  `service_id = "vault-server"`. App dùng nó một lần rồi tự xoá sau khi rotate xong.
